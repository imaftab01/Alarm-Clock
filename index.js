...
// Adding Hours, Minutes, Seconds in DropDown Menu for setting an Alarm
...
// For notifying that Alarm time has reached and its ringing
...
// Displaying Alarms set by users in HTML
...
// Getting alarms from local storage
function fetchAlarm() {
  const alarms = checkAlarams();
...
//Deleting alarm from local storage fuction 
...
// Clearing all the set alarms from local storage as well
const clearAlarmsButton = document.querySelector("#clearAlarmsButton");
clearAlarmsButton.addEventListener("click", clearAlarms);

function clearAlarms() {
  const alarms = document.querySelectorAll(".alarm");
  alarms.forEach((alarm) => {
    const intervalId = alarm.querySelector(".delete-alarm").getAttribute("data-id");
    clearInterval(intervalId);
    alarm.remove();
  });
